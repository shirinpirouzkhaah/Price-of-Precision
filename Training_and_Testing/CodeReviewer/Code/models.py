...
def build_or_load_gen_model(args):
    ...
    # ---- Add special tokens BEFORE loading the model ----
    special_tokens = {
        "additional_special_tokens": ["<START>", "<END>","<msg>", "<keep>", "<add>", "<del>"] + [f"<e{i}>" for i in range(100)]
    }
    num_added_tokens = tokenizer.add_special_tokens(special_tokens)

    # ---- Load the model (allow size mismatch) ----
    ...
    # ---- (Optional) Initialize new token embeddings to mean of existing ones ----
    if num_added_tokens > 0:
        with torch.no_grad():
            input_embeds = model.get_input_embeddings().weight
            old_vocab_size = input_embeds.shape[0] - num_added_tokens
            mean_embed = input_embeds[:old_vocab_size].mean(dim=0)
            input_embeds[old_vocab_size:] = mean_embed

    # ---- Update special token ID mapping ----
    tokenizer.special_dict = {f"<e{i}>": tokenizer.get_vocab()[f"<e{i}>"] for i in range(99, -1, -1)}
    tokenizer.mask_id = tokenizer.get_vocab().get("<mask>", tokenizer.pad_token_id)
    tokenizer.bos_id = tokenizer.get_vocab().get("<s>", tokenizer.cls_token_id)
    tokenizer.pad_id = tokenizer.get_vocab().get("<pad>", tokenizer.pad_token_id)
    tokenizer.eos_id = tokenizer.get_vocab().get("</s>", tokenizer.sep_token_id)
    tokenizer.msg_id = tokenizer.get_vocab().get("<msg>", 0)
    tokenizer.keep_id = tokenizer.get_vocab().get("<keep>", 0)
    tokenizer.add_id = tokenizer.get_vocab().get("<add>", 0)
    tokenizer.del_id = tokenizer.get_vocab().get("<del>", 0)
    tokenizer.start_id = tokenizer.get_vocab().get("<START>", 0)
    tokenizer.end_id = tokenizer.get_vocab().get("<END>", 0)

    logger.info("Finish loading model [%s] from %s", get_model_size(model), args.model_name_or_path)

    # ---- Load checkpoint from saved model if path given ----
    ...
    # Avoid attribute error if local_rank is missing
