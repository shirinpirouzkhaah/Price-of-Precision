...
class RefineDataset(Dataset):         
    def __init__(self, tokenizer, pool, args, dataframe, column_names, samplenum=-1):
        self.tokenizer = tokenizer
        self.args = args
        self.column_names = column_names  # ✅ Fix: Assign column_names
        logger.info("Reading examples from dataframe")

        examples = []
        for i, row in dataframe.iterrows():
            example = {
                "id": row["Record_id"] if "Record_id" in row else i,
                self.column_names[0]: row[self.column_names[0]],
                self.column_names[1]: row[self.column_names[1]],
                self.column_names[2]: row[self.column_names[2]]
            }
        ...
        logger.info("Tokenize examples from dataframe")
        self.feats = pool.map(self.tokenize, [(example, tokenizer, args) for example in examples])

    def tokenize(self, item):
        example, tokenizer, args = item
        old_key, comment_key, new_key = self.column_names

        oldlines = example[old_key].split("\n")
        newlines = example[new_key].split("\n")
        oldlines = [line[1:].strip() for line in oldlines]
        newlines = [line[1:].strip() for line in newlines]
        oldlines = "<add>" + "<add>".join(oldlines)
        newlines = "<add>" + "<add>".join(newlines)
        comment = example[comment_key]
